...
module.exports = [
    {
        id: "fork-banner",
        cause: "n/a",
        summary: "mark the vendored file as a fork and say how to rebase it",
        detail:
            "Keeps the provenance on the file itself, so nobody edits the vendored\n" +
            "grammar thinking it is upstream. Applied like any other patch, so\n" +
            "`build.js --check` still compares vendored == upstream + patches exactly.",
        find: "/**\n * @file Bash grammar for tree-sitter\n",
        replace:
            "/**\n" +
            " * VENDORED FORK — do not edit by hand expecting upstream parity.\n" +
            " *\n" +
            " * This is tree-sitter-bash's grammar with BashTab's patches applied. The\n" +
            " * patch set that derives it lives in lib/grammar/patches.js, and\n" +
            " * `node lib/grammar/build.js --check` asserts this file still equals\n" +
            " * upstream + those patches.\n" +
            " *\n" +
            " * To change the grammar: edit patches.js, run `build.js --rebase`, then\n" +
            " * `test.js`. To adopt a new upstream: `pnpm update tree-sitter-bash`,\n" +
