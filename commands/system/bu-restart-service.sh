...
# Fields: unit action restarted error
# Dispatch: source
...
then
    local _u
    __bu_out_strict_open validation_fd validation_pid "restart-service" '.unit // .name // empty' || { bu_scope_pop_function || true; return 1; }
    while IFS= read -r _u
    do
        [[ -n "$_u" ]] && units+=("$_u")
    done <&"$validation_fd"
